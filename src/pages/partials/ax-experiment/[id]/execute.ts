      ...
      <div class="card" id="cell-${updatedCell.id}">
        <div class="border-border bg-muted flex items-center justify-between border-b px-4 py-2">
          ...
          <div class="flex gap-2">
            <form id="execute-form-${updatedCell.id}" action="/partials/ax-experiment/${updatedCell.id}/execute" method="POST" x-target="cell-${updatedCell.id}">
            ...
            <form action="/partials/ax-experiment/${updatedCell.id}/delete" method="DELETE" x-target="cells-container">
        ...
        <div class="p-4">
          <textarea name="content" form="execute-form-${updatedCell.id}" class="input mb-2 w-full font-mono text-sm" rows="5" placeholder="Enter your signature or code here...">${updatedCell.content || ""}</textarea>
